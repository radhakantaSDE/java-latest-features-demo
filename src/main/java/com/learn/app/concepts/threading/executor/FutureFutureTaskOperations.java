 ...
 * Future<T> is an interface that provides methods to check if the computation is complete, retrieve
 * the result, and cancel the computation. Provides methods to: Check if the computation is complete
 * (isDone()). Cancel the computation (cancel()). Retrieve the result (get()).
 *
 * <p>FutureTask<T> :: FutureTask<T> is a concrete implementation of Future<T> that can be used to
 * wrap a Callable<T> or Runnable. It allows you to create a task that can be executed
 * asynchronously and provides methods to manage the task's execution state.
 */
@Data
@Builder
...
public class FutureFutureTaskOperations {
  ...
  // Ex-1: FutureTask with Runnable. FutureTask can be used to wrap a Runnable or Callable and can
  // be executed by an
