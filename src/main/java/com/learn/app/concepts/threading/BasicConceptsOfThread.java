...
public class BasicConceptsOfThread {
  ...
  public void startThreadTwiceExample() {
    ...
    try {
      ...
      System.out.println(
          "Starting a thread twice failed. Caught IllegalThreadStateException: " + e.getMessage());
    }
  }
  ...
  public void runMethodDirectlyExample() {
    Thread thread3 =
        new Thread(
            () -> {
              System.out.println(
                  "This is the run method being executed directly. Thread name: "
  ...
  public void joinExample() {
        ...
        new Thread(
            () -> {
              try {
                ...
                System.out.println(
                    "T1 : Join thread finished work. Thread name: "
                        + Thread.currentThread().getName());
              } catch (InterruptedException e) {
    ...
    Thread joinThread2 =
        new Thread(
            () -> {
              try {
                System.out.println(
                    "T2 : Join thread finished work. Thread name: "
                        + Thread.currentThread().getName());
              } catch (Exception e) {
                System.out.println("Join thread was interrupted: " + e.getMessage());
              }
    ...
    try {
      ...
      System.out.println(
          "Main thread resumed after joinThread finished. Thread name: "
              + Thread.currentThread().getName());
    } catch (InterruptedException e) {
      System.out.println("Main thread was interrupted: " + e.getMessage());
    }
