...
public class CCompletableFutureChainOperation {
  ...
  public void testThenApply() {
    ...
    CompletableFuture<Integer> cfCallSearchService =
    ...
    CompletableFuture<Integer> cfCallPriceService =
        cfCallSearchService.thenApply(
            searchResult -> {
              int priceRes = searchResult * 10; // Simulated calculation based on search result
              try {
                Thread.sleep(1000); // Simulating a delay
                System.out.println(
                    "Executed Price Result: "
                        + priceRes
                        + "... Thread : "
                        + Thread.currentThread().getName());
              } catch (InterruptedException e) {
  ...
  public void testThenCompose() {
    ...
    CompletableFuture<String> greetingFuture =
        nameFuture.thenCompose(name -> CompletableFuture.supplyAsync(() -> "Hello, " + name + "!"));
