...
public class ArrayListFilterOperation {
  ...
  public void filterEmployeeListAndSortById(List<Employee> employees, String name) {
    ...
    List<Employee> filteredAndSortedEmployees =
        employees.stream()
  ...
  public void findFirstNonRepeatedEmployeeByName(List<Employee> employees) {
    ...
    Employee firstNonRepeated =
        employees.stream()
            .filter(
                e ->
                    employees.stream().filter(emp -> emp.getName().equals(e.getName())).count()
                        == 1)
            .findFirst()
    ...
    List<Employee> nonRepeatedEmployees =
        employees.stream()
            .filter(
                e ->
                    employees.stream().filter(emp -> emp.getName().equals(e.getName())).count()
  ...
  public void findEmployeesByNameRepeatTwice(List<Employee> employees) {
    ...
    List<Employee> repeatedEmployees =
        employees.stream()
            .filter(
                e ->
                    employees.stream().filter(emp -> emp.getName().equals(e.getName())).count()
  ...
  public void findDistinctEmployeesByName(List<Employee> employees) {
    ...
    List<Employee> distinctEmployees =
        employees.stream()
    ...
    List<Employee> distinctEmployeess =
        new ArrayList<>(
            employees.stream()
                .collect(
                    Collectors.toMap(e -> e.getName() + "_" + e.getDept(), e -> e, (e1, e2) -> e1))
                .values());
  ...
  public void groupEmployeesByDepartment(List<Employee> employees) {
    ...
    var groupedByDepartment = employees.stream().collect(Collectors.groupingBy(Employee::getDept));
  ...
  public void sortEmployeesByFrequencyOfName(List<Employee> employees) {
    ...
    List<Employee> sortedByFrequency =
        employees.stream()
            .sorted(
                Comparator.comparingInt(
                        (Employee e) ->
                            (int)
                                employees.stream()
                                    .filter(emp -> emp.getName().equals(e.getName()))
                                    .count())
        ...
        employees.stream()
            ...
            .max(
                Comparator.comparingLong(
                    entry -> entry.getValue())) // Find the entry with the maximum count
            .map(
                entry ->
                    new Employee(
                        0,
                        ...
                        null,
                        null,
                        0)) // Create a new Employee object with the name
            .orElse(null); // Default value if no such employee exists
    ...
    String longestPalindrome =
        strings.stream()
            .filter(
                s ->
                    s.contentEquals(
                        new StringBuilder(s).reverse())) // Check if the string is a palindrome
  ...
  public static void main(String[] args) {
    ...
    List<String> wordsWithCaseSensitive =
        List.of("Hello", "world", "hello", "Java", "java", "WORLD", "hello");
